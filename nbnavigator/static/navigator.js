...
define([
    ...
    CourseList.prototype.handle_load_list = function (data, status, xhr) {
        ...
        if (data.code == 200) {
            this.load_list_success(data.courses);
        } else {
            this.default_course_element.text("Error fetching courses!");
            this.enable_list();
            this.assignment_list.show_error(data.code);
        }
    };
    ...
    CourseList.prototype.change_course = function (course) {
        this.disable_list();
        var class_account = course.class_account;
        var role = course.role;
        var course_id = course.course.id;
        var course_offering = course.course.offering;
        var course_name = course.course.display_name;
        console.log(course_name);
        console.log(course_offering);
        if (this.current_course !== undefined) {
            this.default_course_element.text(course_name);
        }
        this.current_course = course;
        this.default_course_element.text(course_name);//this.default_course_element.text(this.current_course.course.display_name);
        var success = $.proxy(this.load_assignment_list_success, this);
        this.assignment_list.load_list(course_offering, success);
    };
    ...
    CourseList.prototype.load_assignment_list_success = function () {
        if (this.data) {
            ...
            for (var i=0; i<this.data.length; i++) {
                var course_name = this.data[i].course.display_name;
                console.log(course_name);
                var element = $('<li/>').append($('<a/>').attr("href", "#").text(course_name));
    ...
    AssignmentList.prototype.handle_load_list = function (data, status, xhr) {
        console.log(data);
        if (data.code === 200) {
            this.load_list_success(data.assignments);
        } else {
            this.show_error(data.code);
        }
    };
    ...
    Assignment.prototype.make_row = function () {
        ...
        //row.append($('<span/>').addClass('item_course col-sm-6').text(this.data.display_name));
    ...
    Assignment.prototype.make_link = function () {
        ...
        link.text(this.data.display_name);
